...
public class RegisterCritique extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_register_critique);
                        ...
                        if (new_pass.getText().toString().equals(conf_pass.getText().toString())) { // if the passwords matches
                            ...
                            mAuth.createUserWithEmailAndPassword(new_mail.getText().toString(),new_pass.getText().toString()).addOnSuccessListener(new OnSuccessListener<AuthResult>() { // from Firebase create and user with
                                ...
                                public void onSuccess(AuthResult authResult) {
                                    User user = new User(new_username.getText().toString(), new_surname.getText().toString(), true, false);
                                    FirebaseDatabase.getInstance().getReference("User").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).setValue(user).addOnCompleteListener(new OnCompleteListener<Void>() {
                                        ...
                                        public void onComplete(@NonNull Task<Void> task) {
                                            if(task.isSuccessful()){
                                                Toast.makeText(RegisterCritique.this, "Critique Registered Successfully", Toast.LENGTH_LONG).show();
                                                progressBar.setVisibility(View.GONE);
                                                Intent intent = new Intent(RegisterCritique.this, Dashboard.class);
                                                startActivity(intent);
                                            } else {
                                                progressBar.setVisibility(View.GONE);
                                                Toast.makeText(RegisterCritique.this, "Failed to complete", Toast.LENGTH_LONG).show();
                                            }
                                        }
                                    });
                                }
                            ...
                            Toast.makeText(RegisterCritique.this, "The password does not match", Toast.LENGTH_SHORT).show();
                        }
                    } else { // else email is not insert correctly
                        Toast.makeText(RegisterCritique.this, "Invalid email format please insert right email", Toast.LENGTH_SHORT).show();
                    }
                } else { // if all details are not inserted
                    Toast.makeText(RegisterCritique.this, "Please insert your details", Toast.LENGTH_SHORT).show();
                }
            }
        });
